...
public class CourseDBContext extends DBContext<Course> {
    ...
    public Course get(int course_id) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                course.setCourseClass(classses.stream().filter(cl -> cl.getClass_id() == class_id).findAny().get());
            }
  ...
  public ArrayList<Course> registeredCourse(int user_id) {
    ...
    String sql = "SELECT DISTINCT c.course_id, c.course_name, c.class_id, cl.class_name " +
                 "FROM Course c " +
                 "JOIN Class cl ON cl.class_id = c.class_id " +
                 "JOIN Module m ON m.course_id = c.course_id " +
                 "JOIN [dbo].[Lession] l ON l.module_id = m.module_id " +
                 "JOIN [dbo].[Lession_Learn] ll ON l.lession_id = ll.lession_id " +
                 "WHERE ll.user_id = ?";
    ...
    try (PreparedStatement stm = connection.prepareStatement(sql)) {
        ...
        try (ResultSet rs = stm.executeQuery()) {
            while (rs.next()) {
                ...
                c.setCourseClass(new Class(rs.getInt("class_id"), rs.getString("class_name")));
                registeredCourses.add(c);
            }
        }
    } catch (SQLException ex) {
        Logger.getLogger(CourseDBContext.class.getName()).log(Level.SEVERE, "Error fetching registered courses", ex);
    }
